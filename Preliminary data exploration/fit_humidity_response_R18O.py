...
# Log model works better with R
#mod = ExpressionModel('amplitude * ln(x) + bias')
#result_GLW = mod.fit(YVar, x=XVar, amplitude = 1e-7, bias = 1e-2)
...
ax.text(15000, 1.9335e-3, buff_text)
...
# Log model works better with R
mod = ExpressionModel('amplitude * ln(x) + bias')
result_FIN = mod.fit(YVar, x=XVar, amplitude = 1e-7, bias = 1e-2)
...
ax.legend(loc='best')
buff_text = ("R$^2$=%.2f" % r_squared)
ax.text(15000, 1.984e-3, buff_text)
ax.set(ylabel = '$\delta^{18}$O obs-std [‰]', xlabel = 'H$_2$O [ppm]')
ax.grid()
# Print best fit parameters
print('Best fit result ----------------')
...
# Log model works better with R
mod = ExpressionModel('amplitude * ln(x) + bias')
result_BER = mod.fit(YVar, x=XVar, amplitude = 1e-7, bias = 1e-2)
